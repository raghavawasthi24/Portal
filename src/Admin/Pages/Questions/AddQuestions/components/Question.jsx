  ...
  const [ctg,setCtg]=useState("");
    ...
    useEffect(()=>{
      ...
      .then((res)=>{
        ...
        setCount(res.data.msg.categoryResponse[res.data.msg.categoryResponse.length-1].count+1)
      else
      setCount(1)
      }).catch((err)=>{
        console.log(err)
    ...
    useEffect(()=>{
      ...
      if(formvalues.question.trim()!="" && formvalues.opt1.trim()!="" && formvalues.opt2.trim()!="" && formvalues.opt3.trim()!="" && formvalues.opt4.trim()!="" && formvalues.correctAns.trim()!=""){
        axios.post("https://csi-examportal.onrender.com/api/v1/addquestions",{
          ...
          "quesId":`${ctg}`+`${count}`,
          "options":[{"name":formvalues.opt1.trim(),"ansId":`${ctg}`+`${count}`+1},{"name":formvalues.opt2.trim(),"ansId":`${ctg}`+`${count}`+2},{"name":formvalues.opt3.trim(),"ansId":`${ctg}`+`${count}`+3},{"name":formvalues.opt4.trim(),"ansId":`${ctg}`+`${count}`+4}]
        }).then((res)=>{
          console.log(res)
          setCount(count+1)
...
<FormControl fullWidth>
        ...
        <Select
          ...
          variant='standard'
        >
        <MenuItem value={`${ctg}`+`${count}`+1}>{formvalues.opt1}</MenuItem>
        <MenuItem value={`${ctg}`+`${count}`+2}>{formvalues.opt2}</MenuItem>
        <MenuItem value={`${ctg}`+`${count}`+3}>{formvalues.opt3}</MenuItem>
        <MenuItem value={`${ctg}`+`${count}`+4}>{formvalues.opt4}</MenuItem>
        </Select>
      </FormControl>
